...
class Caller(Chain, BaseModel):
    ...
    #     if isinstance(llm_output, dict):  # ✅ Ensure llm_output is a string
    #         llm_output = llm_output.get("result", "")
    #     match = re.search(regex, llm_output, re.DOTALL)  # ✅ Now llm_output is always a string
    #     if not match:
    ...
    def _get_action_and_input(self, llm_output: Union[str, AIMessage]) -> Tuple[str, str]:
        ...
        # ✅ Regex pattern for extracting operation & input
        regex = r"Operation:[\s]*(.*?)[\n]*Input:[\s]*(.*)"        
        match = re.search(regex, llm_output, re.DOTALL)
    ...
    def _call(self, inputs: Dict[str, str]) -> Dict[str, str]:
        ...
        matched_endpoints = get_matched_endpoint(self.api_spec, api_plan["result"])
        ...
        #print(f"DEBUG: Matched Endpoints: {matched_endpoints}")  # 🔍 Debugging
        #assert len(matched_endpoints) == 1, f"Found {len(matched_endpoints)} matched endpoints, but expected 1."
        if len(matched_endpoints) > 1:
            print(f"⚠️ WARNING: Found multiple matched endpoints: {matched_endpoints}. Selecting the most relevant one.")

        # ✅ Choose the most relevant endpoint (modify as needed)
        selected_endpoint = matched_endpoints[0]  # Pick the first match (you can refine this)

        # If no exact match, log and return error
        if not selected_endpoint:
            raise ValueError("❌ Error: No valid API endpoint found.")

        print(f"✅ Using API Endpoint: {selected_endpoint}")
        ...
        while self._should_continue(iterations, time_elapsed):
            ...
            caller_chain_output = caller_chain.invoke({"api_plan": api_plan, "background": inputs['background'], "agent_scratchpad": scratchpad, "stop": self._stop})
        ...
        return {"result": caller_chain_output.content.strip()}  # ✅ Extract content from AIMessage